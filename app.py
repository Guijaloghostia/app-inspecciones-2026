...
# ARCHIVO PREDETERMINADO CONFIGURADO CON TU NUEVA BASE 2026:
...
# --- FUNCIÓN CARGA DE DATOS ROBUSTA ---
def cargar_datos(file_source):
    excel_file = pd.ExcelFile(file_source, engine='openpyxl')
    
    # Toma la pestaña TOTAL o la primera disponible
    sheet = 'TOTAL' if 'TOTAL' in excel_file.sheet_names else excel_file.sheet_names[0]
...
if uploaded_file is not None:
    ...
    st.sidebar.info("Usando base predeterminada")
...
if resumen is not None:
    ...
    # --- SECCIÓN 6: CARGA Y CONFIGURACIÓN ---
        ...
        st.info("💡 Consejo: Podés subir un archivo Excel desde el selector del menú lateral izquierdo.")
else:
