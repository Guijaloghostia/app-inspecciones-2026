...
def check_password():
    ...
    def password_entered():
        ...
        if username in allowed_users and str(allowed_users[username]) == str(password):
...
if resumen is not None:
    ...
    # --- SECCIÓN 6: TABLERO DE PRIORIDADES ---
        ...
        st.title("🔴 Tablero de Prioridades de Refiscalización")
        st.write("Listado de domicilios críticos que requieren una nueva visita.")

        filtro_prio = st.selectbox(
            "Filtrar por Nivel de Prioridad:",
            ["Todos", "🔴 ALTA (1 sola insp. e irregular)", "🟡 MEDIA (1-2 inspecciones)", "🟢 BAJA (3+ inspecciones)"]
        )

        df_prio = resumen.copy()
        if filtro_prio != "Todos":
            df_prio = df_prio[df_prio["Prioridad"] == filtro_prio]
    ...
    # --- SECCIÓN 7: MAPA DE CONTROL ---
        ...
        st.title("🗺️ Mapa de Calor y Ubicación de Fiscalizaciones")
        st.write("Visualización geográfica de los domicilios inspeccionados.")

        coords_cache = geocodificar_direcciones_seguro(resumen)
        ...
        for _, row in resumen.iterrows():
            dir_corta = row["Direccion_Corta"]
            if dir_corta in coords_cache:
                lat, lon = coords_cache[dir_corta]
                if lat and lon:
                    heat_data.append([lat, lon])
                    folium.CircleMarker(
                        location=[lat, lon],
                        radius=5,
                        color="red" if "ALTA" in str(row["Prioridad"]) else "blue",
                        fill=True,
                        fill_color="red" if "ALTA" in str(row["Prioridad"]) else "blue",
                        popup=f"{dir_corta} - {row['Razon_Social']}",
                    ).add_to(m)
    ...
    # --- SECCIÓN 8: CARGA Y CONFIGURACIÓN ---
        ...
        st.write("Detalles técnicos de las bases de datos cargadas en memoria.")

        st.metric("Total Registros Base 2026", len(df_raw))
        if resumen_2025 is not None:
            st.success(f"Base 2025 cargada correctamente con {len(resumen_2025)} registros.")
        else:
            st.warning("No se detectó la base 2025 fija.")
